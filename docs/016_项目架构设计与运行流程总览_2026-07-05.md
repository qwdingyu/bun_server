...
- `AuthController`：只承载本地认证闭环，包括注册、登录、refresh token 轮换与登出撤销会话；继续保持 `/api/users/auth/*` URL 契约不变。
- `UserController`：继续承载用户资料、用户管理、批量操作、当前用户权限菜单等后台高频用例，暂不为了“整洁”继续拆成多个小控制器。
- `RbacController`：承载角色、权限、用户授权关系的轻量管理接口，不扩展成重型 IAM。
- 后续只有当认证、用户管理、RBAC 或新增业务资源出现明确变化原因冲突时，才继续小步拆分。
